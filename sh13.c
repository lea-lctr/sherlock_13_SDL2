...
int main(int argc, char **argv) {
  ...
  while (!quit) {
    if (SDL_PollEvent(&event)) {
      ...
      case SDL_MOUSEBUTTONDOWN:
        ...
        if ((mx < 200) && (my < 50) && (connectEnabled == 1)) {
          sprintf(sendBuffer, "C %s %d %s", gClientIpAddress, gClientPort,
                  gName);
    ...
    if (synchro == 1) {
      printf("consomme |%s|\n", gbuffer);
      switch (gbuffer[0]) {
      // Message 'I' : le joueur recoit son Id sous la forme "I id"
      case 'I':
        // RAJOUTER DU CODE ICI
        gId = atoi(gbuffer + 2);
        break;
      ...
      case 'L':
        // RAJOUTER DU CODE ICI
        // copie de gbuffer car strtok modifie la chaîne
        char bufcpy[512];
        strncpy(bufcpy, gbuffer, sizeof(bufcpy) - 1);
        bufcpy[sizeof(bufcpy) - 1] = '\0';

        // on découpe par espaces
        char *tok = strtok(bufcpy, " \t\r\n");

        // si tok[0] n'est pas 'L', on sort
        if (!tok || tok[0] != 'L')
          break;

        // on parcourt les 4 suivants
        for (int i = 0; i < 4; i++) {
          tok = strtok(NULL, " \t\r\n");
          if (!tok)
            break;
          // si ce n'est pas un placeholder "-"
          if (tok[0] != '-' || tok[1] != '\0') {
            // on met à jour gNames[i]
            strncpy(gNames[i], tok, 255);
            gNames[i][255] = '\0';
          }
        }
        break;
      ...
      case 'D':
        // RAJOUTER DU CODE ICI
		// On recoit les id des cartes de cette façon "D %d %d %d" pour être stocké dans b[3]
		int n = sscanf(gbuffer, "D %d %d %d", &b[0], &b[1], &b[2]);
		if (n != 3) {
			// Si on n'a pas lu exactement 3 entiers, gérer l'erreur
			fprintf(stderr, "Erreur de format sur le message D : reçu '%s'\n", gbuffer);
		}
