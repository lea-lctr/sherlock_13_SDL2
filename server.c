...
// this array is indexed by the number stored on deck arry
char *nomcartes[] = {
...
void printClients() {
  ...
  for (i = 0; i < nbClients; i++)
    printf("%d: %s %5.5d %s\n", i, tcpClients[i].ipAddress, tcpClients[i].port,
           tcpClients[i].name);
}
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (fsmServer == 0) {
      switch (buffer[0]) {
      case 'C':
        sscanf(buffer, "%c %s %d %s", &com, clientIpAddress, &clientPort,
               clientName);
        ...
        sendMessageToClient(tcpClients[id].ipAddress, tcpClients[id].port,
                            reply);

        // Envoyer un message broadcast pour communiquer a tout le monde la
        // liste des joueurs actuellement connectes

        sprintf(reply, "L %s %s %s %s", tcpClients[0].name, tcpClients[1].name,
        ...
        if (nbClients == 4) {
          ...
          // On envoie ses cartes au joueur 0, ainsi que la ligne qui lui
          ...
          // On envoie ses cartes au joueur 1, ainsi que la ligne qui lui
          // correspond dans tableCartes RAJOUTER DU CODE ICI
		  sprintf(reply, "D %d %d %d", deck[3], deck[4], deck[5]);
		  sendMessageToClient(tcpClients[1].ipAddress, tcpClients[1].port, reply);

          // On envoie ses cartes au joueur 2, ainsi que la ligne qui lui
          // correspond dans tableCartes RAJOUTER DU CODE ICI
		  sprintf(reply, "D %d %d %d", deck[6], deck[7], deck[8]);
		  sendMessageToClient(tcpClients[2].ipAddress, tcpClients[2].port, reply);

          // On envoie ses cartes au joueur 3, ainsi que la ligne qui lui
          // correspond dans tableCartes RAJOUTER DU CODE ICI
		  sprintf(reply, "D %d %d %d", deck[9], deck[10], deck[11]);
		  sendMessageToClient(tcpClients[3].ipAddress, tcpClients[3].port, reply);

          // On envoie enfin un message a tout le monde pour definir qui est le
          // joueur courant=0 RAJOUTER DU CODE ICI
